...
\subsection{IQL}
A natural way to model a multi-agent system is to define a cost function $J_{i}(\theta)$ for each agent $i \in N$. The agent $i$ is then allowed to minimize $J_{i}(\theta)$ in order to optimize its expected payoffs $Q_{i}(u^{(i)},s;\theta) = \mathbb{E}_{\pi_{\theta}}[\sum_{t=1}^{T}\gamma^{t}r(s,u)|s=s_{t},u^{(i)}=u^{(i)}_{t}]$. This form of the reward maximization is directly based on the standard Q-learning as exprresed in \cite{eq:action-value}. Each agent maintains its set of Q-values which are directly affected only by the agent's actions $u^{(i)}_{t}$ in a given state $s_{t}$. 
